...
cv.resizeWindow('first', 800, 600)
cv.namedWindow('sec', cv.WINDOW_NORMAL)
cv.resizeWindow('sec', 800, 600)
cv.namedWindow('third', cv.WINDOW_NORMAL)
cv.resizeWindow('third', 800, 600)
cv.namedWindow('count', cv.WINDOW_NORMAL)
cv.resizeWindow('count', 800, 600)
...
# task4:提取数字
hsv = cv.cvtColor(res3, cv.COLOR_BGR2HSV)
...
fourth = res[y:y+h , x:x+w]


# 输出
# cv.imwrite('try_3.jpg', res3)
# cv.imshow('img', img)
# cv.imshow('res1', res1)
# cv.imshow('res2', res2)
